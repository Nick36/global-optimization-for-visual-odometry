...
FullSystem::FullSystem()
{
    ...
    std::string vocPath (PATH_TO_VOC);
    bool loadSuccess = voc->loadFromTextFile(vocPath);
    if(!loadSuccess)
    {
        std::cerr << "Wrong path to vocabulary. " << std::endl;
        std::cerr << "Falied to open at: " << PATH_TO_VOC << std::endl;
        exit(-1);
    }
    std::cout << "Vocabulary loaded!" << std::endl;

    std::cout << "Creating the keyframe database..." << std::endl;

    // last parameter sets number of levels to go up the vocabulary tree to select the
    // node id to store in the direct index when adding images
    db = new KeyframeDatabase(*voc, true, 0);

    std::cout << "Keyframe database created!" << std::endl;

    std::cout << "Initializing the ORB features extractor..." << std::endl;

    ext = new ORBextractor(int(ORB_EXTRACTOR_NUMBER_OF_FEATURES), float(ORB_EXTRACTOR_SCALE_FACTOR),
                                   int(ORB_EXTRACTOR_NUMBER_OF_LEVELS), int(ORB_EXTRACTOR_INITIAL_THRESHOLD_FAST),
                                   int(ORB_EXTRACTOR_MINIMAL_THRESHOLD_FAST));

    std::cout << "ORB features extractor initialized!" << std::endl;

    // GTSAM
    // In batch mode, ISAM2 performs partial relinearization/reordering at each step.
    // The relinearization threshold is set small so that the ISAM2 result approaches the batch result.

    if (setting_iSAM2batchMode)
    {
        gtsam::ISAM2Params parameters;
        parameters.relinearizeThreshold = 0.01;
        parameters.relinearizeSkip = 1;
        // do not reference result members if evaluateNonlinearError is not set!
        //parameters.evaluateNonlinearError = true;
        isam_ = gtsam::ISAM2(parameters);
    }
    else isam_ = gtsam::ISAM2();

    performISAMUpdate_ = false;
...
//        allPhotometricFactors.push_back(gtsam::NonlinearFactorGraph());
}
...
FullSystem::~FullSystem()
{
	...
	delete ef;
    for(FrameHessian* mfh :marginalizedFrameHessians)
        delete mfh;
...
void FullSystem::printResult(std::string file, string optimized_file)
{
	...
	std::ofstream myfile;
    myfile.open ((strTime + file).c_str());
	myfile << std::setprecision(15);

    std::ofstream my_optimized_file;
    my_optimized_file.open ((strTime + optimized_file).c_str());
    my_optimized_file << std::setprecision(15);
    ...
    for (int i; i < marginalizedFrameHessians.size(); i++)
        frameShellIDtoOptimizedKeyframePose.insert(std::make_pair(marginalizedFrameHessians[i]->shell->id, framePoses[i]));
	...
	for(FrameShell* s : allFrameHistory)
	{
        ...
        std::map <int, SE3>::iterator it = frameShellIDtoOptimizedKeyframePose.find(s->id);
        assert(it != frameShellIDtoOptimizedKeyframePose.end());
        SE3 optimized_pose = it->second;

        my_optimized_file << s->timestamp <<
            " " << optimized_pose.translation().transpose()<<
            " " << optimized_pose.unit_quaternion().x()<<
            " " << optimized_pose.unit_quaternion().y()<<
            " " << optimized_pose.unit_quaternion().z()<<
            " " << optimized_pose.unit_quaternion().w() << "\n";
	}
	myfile.close();
    my_optimized_file.close();
}
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
	...
	for(unsigned int i=0;i<lastF_2_fh_tries.size();i++)
	{
		...
		bool trackingIsGood = coarseTracker->trackNewestCoarse(
                ...
                //dsslam
                //pyrLevelsUsed-1,
                pyrLevelsUsed > 5 ? 4 : pyrLevelsUsed-1,
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	for(FrameHessian* fh1 : frameHessians)		// go through all active frames
	{
		if(fh1 == fh) continue;
        //DSSLAM
        localOptimizationWindowKeyframes[fh->frameID].push_back(fh1->frameID);
        localOptimizationWindowKeyframes[fh1->frameID].push_back(fh->frameID);
