...
void CoarseInitializer::compOptSE3(FrameHessian* reference, FrameHessian* target, CalibHessian* HCalib,
                                   const Eigen::Ref<const Mat33>& rot)
{
    makeK(HCalib);
    firstFrame = reference;
    newFrame = target;
    //int maxIterations[] = {5,5,10,30,50,50};
    int maxIterations[] = {20,20,40,60,100,100}; //  first three multiplied by 4, next two multiplied by 2 compared to trackFrame, added last one

    Eigen::DiagonalMatrix<float, 6> wMalt;
    wMalt.diagonal()[0] = wMalt.diagonal()[1] = wMalt.diagonal()[2] = SCALE_XI_ROT;
    wMalt.diagonal()[3] = wMalt.diagonal()[4] = wMalt.diagonal()[5] = SCALE_XI_TRANS;

    for (int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
    {
        int npts = reference->pointHessiansMarginalized.size(); // int npts = numPoints[lvl];
        numPoints[lvl] = npts;
        Pnt* pts = new Pnt[npts]; // Pnt* pts = points[lvl];
        points[lvl] = pts;
        for(int i=0;i<npts;i++)
        {
            pts[i].u = ((float) pow(0.5, lvl)) * (reference->pointHessiansMarginalized[i]->u);
            pts[i].v = ((float) pow(0.5, lvl)) * (reference->pointHessiansMarginalized[i]->v);
            pts[i].idepth = pts[i].idepth_new = reference->pointHessiansMarginalized[i]->idepth_scaled;
            pts[i].isGood = true;
            pts[i].energy.setZero();
            pts[i].outlierTH = patternNum*setting_outlierTH;
        }
    }

    SE3 refToNew_current = SE3(rot, Mat31());
    refToNew_current.translation().setZero();

    Vec2f affL = reference->targetPrecalc[target->idx].PRE_aff_mode;
    ...
    for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
    {
        ...
        Pnt* pts = points[lvl];
        Mat66f H; Vec6f b;
        eTotalOld = gaussNewton(lvl, H, b, teeOld, refToNew_current, affL);
        ...
        while(true)
        {
            for(int i=0;i<6;i++) H(i,i) *= (1+lambda); // Levenberg-Marquardt

            H = wMalt * H * wMalt * (0.01f/(w[lvl]*h[lvl]));
            if (H.norm() < 1e-35)  {
                cout << "H matrix has too small norm" << endl;
                break;
            }
            b = wMalt * b * (0.01f/(w[lvl]*h[lvl]));
            Vec6f inc = - (wMalt * (H.ldlt().solve(b)));	//=-H^-1 * b

            assert(inc.allFinite());
            ...
            Vec6f b_new;
            size_t teeNew = 0;
            float eTotalNew = gaussNewton(lvl, H_new, b_new, teeNew, refToNew_new, affL);

            // accept last Gauss-Newton step
            if(eTotalOld > eTotalNew)
            {
                eTotalOld = eTotalNew;
                H = H_new;
                b = b_new;
            ...
            if(!(inc.norm() > eps)) break;

            if(iteration >= maxIterations[lvl]) {
                cout << "iterations limit reached" << endl;
                break;
            }

            if(fails >= 5) {
                cout << "fails limit reached" << endl;
                break;
            }
...
float CoarseInitializer::gaussNewton(int lvl, Mat66f &H_out, Vec6f &b_out, size_t &true_energy_entries_out, const SE3 &refToNew, Vec2f affL)
{
    ...
    for(int i=0;i<npts;i++)
    {
        ...
        if(!(point->isGood && point->u > 1 && point->v > 1 && point->u < wl-3 && point->v < hl-3))
        {
            energyTotal += (float)(point->energy[0]);
        ...
        for(int idx=0;idx<patternNum;idx++)
        {
            ...
            // we have an offset of maximum two in the residual pattern
            // we also use the pixels one to the right, one to the bottom and one to both the right and the bottom
            if(!(Ku > 1 && Kv > 1 && Ku < wl-3 && Kv < hl-3 && new_idepth > 0))
            {
                isGood = false;
                break;
            }
        ...
        // update Hessian matrix and residual vector.
        for(int idx=0;idx<patternNum;idx++)
        {
            Vec6f dp;
            dp << (float)dp0[idx],(float)dp1[idx],(float)dp2[idx],(float)dp3[idx],(float)dp4[idx],(float)dp5[idx];
            assert(dp.allFinite());
            Eigen::Matrix<float,1,6> dpTr = dp.transpose();
            Mat66f matInc = dp * dpTr;

            // BEGIN DEBUG
            assert(matInc.allFinite());
            bool symmetrical = true;
            for (int k = 0; k<6; k++)
                for (int l = k; l<6; l++)
                    if (matInc(k,l) != matInc(l,k))
                        symmetrical = false;
            assert(symmetrical);
            // END DEBUG
    ...
    assert (H_out.allFinite());
    assert (b_out.allFinite());
    assert (!isnan(energyTotal));
    assert (energyTotal >= 0);
    assert (trueEnergyEntries >= 0);

    true_energy_entries_out = trueEnergyEntries;
