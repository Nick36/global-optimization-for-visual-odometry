...
void PangolinDSOViewer::run()
{
    ...
    pangolin::Var<bool> settings_showOptimizedKFCameras("ui.OptKFCam",false,true);
    ...
    pangolin::Var<bool> settings_showOptimizedTrajectory("ui.OptimizedTraj",false,true);
    ...
    pangolin::Var<bool> settings_showAllGTSAMPhotometricFactors("ui.AllGTSAMPhFactors",false,true);
    pangolin::Var<bool> settings_showAllGTSAMBetweenFactors("ui.AllGTSAMBetFactors",false,true);
    pangolin::Var<bool> settings_showAllGTSAMBetweenConstraints("ui.AllGTSAMBetConst",false,true);
    pangolin::Var<bool> settings_showFirstLast("ui.First&LastFrame",false,true);
    pangolin::Var<bool> settings_showPointCloud("ui.PointCloud",true,true);
    pangolin::Var<bool> settings_showOptimizedPointCloud("ui.OptPointCloud",false,true);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		if(setting_render_display3D)
		{
			...
			{
                if (settings_showFirstLast && (counter == 0 || counter == keyframes.size()-1)) fh->drawCam(1,colors[CYAN],0.1);

                if(this->settings_showKFCameras) fh->drawCam(1,colors[BLUE],0.1);

                refreshed =+ (int)(fh->refreshPC(refreshed < 10, this->settings_scaledVarTH, this->settings_absVarTH,
                                                 this->settings_pointCloudMode, this->settings_minRelBS, this->settings_sparsity));
                if(this->settings_showPointCloud)
                    fh->drawPC(1);

                if(this->settings_showOptimizedPointCloud && frameShellIDtoOptimizedKeyframePose.count(fh->id))
                    fh->drawOptimalPC(1,frameShellIDtoOptimizedKeyframePose[fh->id]);

                counter++;
			}

            counter = 0;
            for(KeyFrameDisplay* fh : optimizedKeyframes)
            {
                if (settings_showFirstLast && (fh->id == 0 || counter == optimizedKeyframes.size()-1)) fh->drawCam(1,colors[MAGENTA],0.1);
                if(this->settings_showOptimizedKFCameras) fh->drawCam(1,colors[GREEN],0.1);
                counter++;
            }
...
//            {
//                unsigned int i = 0;
//                for (KeyFrameDisplay* cp : camPoses) {
//                    cp->drawCam(1, colors[camPosesColors[i]], 0.1);
//                    i++;
//                }
//            }
        ...
        this->settings_showAllGTSAMPhotometricFactors = settings_showAllGTSAMPhotometricFactors.Get();
        this->settings_showAllGTSAMBeweenFactors = settings_showAllGTSAMBetweenFactors.Get();
        this->settings_showAllGTSAMBeweenConstraints = settings_showAllGTSAMBetweenConstraints.Get();
        ...
        this->settings_showOptimizedKFCameras = settings_showOptimizedKFCameras.Get();
	    this->settings_showTrajectory = settings_showTrajectory.Get();
        this->settings_showOptimizedTrajectory = settings_showOptimizedTrajectory.Get();
        this->settings_showPointCloud = settings_showPointCloud.Get();
        this->settings_showOptimizedPointCloud = settings_showOptimizedPointCloud.Get();
        this->settings_showFirstLast = settings_showFirstLast.Get();
...
void PangolinDSOViewer::reset_internal()
{
	...
	keyframes.clear();
    for(size_t i=0; i<optimizedKeyframes.size();i++) delete optimizedKeyframes[i];
    optimizedKeyframes.clear();
    photometricFactorLines.clear();
    betweenFactorLines.clear();
    betweenConstraintLines.clear();
    clearCamPosesMap();
	allFramePoses.clear();
    frameIDtoOptimizedFramePose.clear();
...
void PangolinDSOViewer::drawConstraints()
{
    ...
    if (settings_showAllGTSAMPhotometricFactors)
    {
        for (auto line : photometricFactorLines)
        {
            glLineWidth(1);
            glColor3f(1,0.5,0);
            glBegin(GL_LINES);
            glVertex3f((GLfloat) line.first[0],(GLfloat) line.first[1], (GLfloat) line.first[2]);
            glVertex3f((GLfloat) line.second[0],(GLfloat) line.second[1], (GLfloat) line.second[2]);
            glEnd();
        }
    }

    if (settings_showAllGTSAMBeweenFactors)
    {
        for (auto line : betweenFactorLines)
        {
            glLineWidth(1);
            glColor3f(0,1,1);
            glBegin(GL_LINES);
    ...
    if (settings_showAllGTSAMBeweenConstraints)
    {
        for (auto line : betweenConstraintLines)
        {
            glLineWidth(1);
            glColor3f(1,0,1);
            glBegin(GL_LINES);
...
//		glLineWidth(3);
    ...
    if(settings_showOptimizedTrajectory)
	{
        ...
        glBegin(GL_LINE_STRIP);
        for(auto el : frameIDtoOptimizedFramePose)
        {
            glVertex3f((float)el.second[0],
                    (float)el.second[1],
                    (float)el.second[2]);
        }
        glEnd();
...
void PangolinDSOViewer::drawCamPoses(std::vector<FrameHessian*> &frames, std::vector<SE3> &frameTransformations, std::vector<Color> &frameColors, CalibHessian* HCalib)
{
    assert(frames.size() == frameTransformations.size() && frames.size() == frameColors.size());
    ...
    for (int i; i < frames.size(); i++){
        KeyFrameDisplay* kfd = new KeyFrameDisplay();
        kfd->setFromF(frames[i]->shell, HCalib);
        kfd->camToWorld =  kfd->camToWorld * frameTransformations[i];
        camPoses.push_back(kfd);
        camPosesColors.push_back(frameColors[i]);
    }
}

void PangolinDSOViewer::clearCamPosesMap() {
    for(KeyFrameDisplay* cp : camPoses) {
        delete cp;
    }
    camPoses.clear();
    camPosesColors.clear();
}

void PangolinDSOViewer::drawPhotometricFactor(FrameHessian* from, FrameHessian* to)
{
    Vec3f p1 = from->shell->camToWorld.translation().cast<float>();
    Vec3f p2 = to->shell->camToWorld.translation().cast<float>();
    photometricFactorLines.push_back(std::make_pair(p1,p2));

}

void PangolinDSOViewer::drawBetweenFactor(FrameHessian* from, FrameHessian* to)
{
    Vec3f p1 = from->shell->camToWorld.translation().cast<float>();
    Vec3f p2 = to->shell->camToWorld.translation().cast<float>();
    betweenFactorLines.push_back(std::make_pair(p1,p2));

}

void PangolinDSOViewer::drawBetweenConstraint(FrameHessian* from, FrameHessian* to)
{
    Vec3f p1 = from->shell->camToWorld.translation().cast<float>();
    Vec3f p2 = to->shell->camToWorld.translation().cast<float>();
    betweenConstraintLines.push_back(std::make_pair(p1,p2));

}

void PangolinDSOViewer::publishOptimizedKeyframes(
        std::vector<FrameHessian*> &frames,
        std::vector<SE3> &framePoses,
        CalibHessian* HCalib)
{
    assert(frames.size() == framePoses.size());
    ...
    // clear old estimates for optimal camera poses
    for(KeyFrameDisplay* okf : optimizedKeyframes) delete okf;
    optimizedKeyframes.clear();
    frameShellIDtoOptimizedKeyframePose.clear();
    frameIDtoOptimizedFramePose.clear();
    ...
    for (int i; i < frames.size(); i++)
    {
        KeyFrameDisplay* kfd = new KeyFrameDisplay();
        kfd->setFromF(frames[i]->shell, HCalib);
        kfd->camToWorld = framePoses[i];
        optimizedKeyframes.push_back(kfd);
        // add a <keyframe shell id, camera pose optimized by GTSAM> pair to corresponding map
        frameShellIDtoOptimizedKeyframePose.insert(std::make_pair(frames[i]->shell->id, framePoses[i]));
        frameIDtoOptimizedFramePose.insert(std::make_pair(frames[i]->frameID, framePoses[i].translation().cast<float>()));
    }
}
